...
def make_folder():
    return ssh_checkout(data["ip"], data["user"], data["passwd"],
                        f'mkdir -p {data["folder_in"]} {data["folder_out"]} {data["folder_ext"]} {data["folder_ext3"]} '
                        f'{data["folder_bad"]} {data["stat_file_dir"]}', "")
...
def clear_folder():
    return ssh_checkout(data["ip"], data["user"], data["passwd"],
                        ...
                        f'{data["folder_ext3"]}/* {data["folder_bad"]}/*', "")
...
def make_files():
    ...
    for i in range(data['count']):
        ...
        ssh_checkout(data["ip"], data["user"], data["passwd"], f'cd {data["folder_in"]}; dd if=/dev/urandom '
                                                               f'of={file_name} bs={data["bs"]} '
...
def create_bad_archive():
    ssh_checkout(data["ip"], data["user"], data["passwd"], f'cd {data["folder_in"]}; 7z a -t{data["arc_type"]} '
                                                           f'{data["folder_out"]}/arx2', "Everything is Ok")
    ssh_checkout(data["ip"], data["user"], data["passwd"], f'cp {data["folder_out"]}/arx2.{data["arc_type"]} '
                                                           f'{data["folder_bad"]}', '')
    ssh_checkout(data["ip"], data["user"], data["passwd"], f'truncate -s 1 {data["folder_bad"]}/arx2.'
                                                           f'{data["arc_type"]}', '')  # сделали битым
...
def stat_fixture():
    ...
    ssh_checkout(data["ip"], data["user"], data["passwd"], f'cd {data["stat_file_dir"]}; '
                                                           f'echo "time = {time_stamp}, '
